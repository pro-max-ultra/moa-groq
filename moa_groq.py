...
class Pipeline:
    ...
    async def call_reference_models_parallel(self, messages):
        ...
        for model in self.reference_models:
            logger.info(
                f"Using reference model: {model['name']} with API base: {model['api_base']} and API key: {model['api_key']}")
    ...
    async def run_pipeline(self, user_message, temperature=GROQ_DEFAULT_TEMPERATURE, max_tokens=GROQ_DEFAULT_MAX_TOKENS,
        ...
        self.messages.append(
            {"role": "assistant", "content": output})  # Append the assistant's response to the conversation history
